...
@export var bounds: Array[Vector4] = [Vector4(), Vector4(), Vector4(), Vector4()]
...
func set_bounds(depth: int, value: Vector4):
	bounds[depth] = value
	RenderingServer.global_shader_parameter_set("mask_bounds", Projection(bounds[0], bounds[1], bounds[2], bounds[3]))
